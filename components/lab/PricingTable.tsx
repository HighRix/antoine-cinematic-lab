...
export function PricingTable() {
  return (
    <div className="grid grid-cols-1 md:grid-cols-3 gap-px bg-white/8 border border-white/8 rounded-[20px] overflow-hidden">
      {ORDER.map((key) => {
        ...
        return (
          <div
            key={key}
            className={`relative bg-[#0C0C0C] p-7 md:p-9 flex flex-col gap-6 ${
              isFeatured ? 'ring-1 ring-[#F27D26]/55 ring-inset' : ''
            }`}
          >
            {'badge' in tier && tier.badge && (
              <span className="absolute -top-3 left-7 inline-flex items-center text-[10px] font-bold tracking-[0.22em] uppercase font-mono bg-[#F27D26] text-[#0C0C0C] px-3 py-1.5 rounded-full">
                {tier.badge}
              </span>
            )}
            ...
            <div>
              ...
              <div className="mt-4 flex items-baseline gap-2 flex-wrap">
                {'anchorPrice' in tier && tier.anchorPrice && (
                  <span className="text-white/30 text-xl line-through font-serif italic">
                    €{tier.anchorPrice}
                  </span>
                )}
                <span className="font-serif italic text-[44px] md:text-[52px] text-white tracking-[-0.02em] leading-none">
                  €{tier.price}
                </span>
            ...
            <a
              ...
              className={`inline-flex items-center justify-center gap-2 rounded-full px-5 py-3 text-[14px] font-medium transition-all hover:scale-[1.02] ${
                isFeatured
                  ? 'bg-[#F27D26] hover:bg-[#FF8A2E] text-[#0C0C0C] shadow-[0_8px_24px_rgba(242,125,38,0.35)]'
